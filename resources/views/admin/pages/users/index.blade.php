                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($users as $user)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                @if (request('role') == 'guru')
                                    <td class="px-6 py-4 text-gray-500">{{ $user->guru->nip ?? '-' }}</td>
                                    <td class="px-6 py-4 text-gray-500">{{ $user->guru->mata_pelajaran ?? '-' }}</td>
                                @elseif(request('role') == 'siswa')
                                    <td class="px-6 py-4 text-gray-500">{{ $user->siswa->nis ?? '-' }}</td>
                                    <td class="px-6 py-4 text-gray-500">{{ $user->siswa->kelas->nama_kelas ?? '-' }}</td>
                                    <td class="px-6 py-4 text-gray-500">{{ $user->siswa->jurusan->nama_jurusan ?? '-' }}
                                    </td>
                                @endif
                    ...
                    <div class="flex items-center mb-2">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500">{{ $user->username }} •
                                {{ ucfirst($user->role->nama_role ?? '-') }}</p>
                        </div>
                    </div>
                    <div class="text-sm text-gray-600 space-y-1">
                        @if (request('role') == 'guru')
                            <p><span class="font-medium">NIP:</span> {{ $user->guru->nip ?? '-' }}</p>
                            <p><span class="font-medium">Mapel:</span> {{ $user->guru->mata_pelajaran ?? '-' }}</p>
                        @elseif(request('role') == 'siswa')
                            <p><span class="font-medium">NIS:</span> {{ $user->siswa->nis ?? '-' }}</p>
                            <p><span class="font-medium">Kelas:</span> {{ $user->siswa->kelas->nama_kelas ?? '-' }}</p>
                            <p><span class="font-medium">Jurusan:</span> {{ $user->siswa->jurusan->nama_jurusan ?? '-' }}
                            </p>
                        @endif
                        <p><span class="font-medium">Dibuat:</span> {{ $user->created_at->format('d M Y') }}</p>
                    </div>
                    <div class="flex justify-end mt-3 space-x-3">
                        @if (
                            $user->role_id !== \App\Http\Controllers\Admin\UserController::ROLE_SUPER_ADMIN ||
                                auth()->user()->role_id === \App\Http\Controllers\Admin\UserController::ROLE_SUPER_ADMIN)
                            <button onclick='openEditModal(@json($user))' class="text-amber-600">
                                <i class="fas fa-edit"></i>
                            </button>
                        @endif
                        @if (
