...
@section('content')
    <div class="space-y-6 font-poppins">
        ...
        {{-- Tabel Data --}} <div class="bg-white rounded-xl shadow border border-gray-100 hidden sm:block">
                        ...
                        @forelse ($users as $user)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4">
                                    @if ($user->role)
                                        <span
                                            class="px-2 py-1 text-xs font-semibold rounded-full @if ($user->role->nama_role == 'admin') bg-red-100 text-red-800 @elseif($user->role->nama_role == 'guru') bg-yellow-100 text-yellow-800 @else bg-blue-100 text-blue-800 @endif">
                                            {{ ucfirst($user->role->nama_role) }} </span>
                                    @else
                                        <span class="text-gray-400">-</span>
                                        @endif
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-500">{{ $user->created_at->format('d M Y') }}</td>
                                <td class="px-6 py-4 text-right space-x-3"> <a
                                        href="{{ route('admin.users.edit', $user->id) }}"
                                        class="text-amber-600 hover:text-amber-800"> <i class="fas fa-edit"></i> </a>
                                    @if ($user->id !== auth()->id())
    ...
    @push('scripts')
        <script>
            ...
            document.getElementById('role').addEventListener('change', toggleSiswaFilters);
            toggleSiswaFilters();
        </script>
    @endpush
